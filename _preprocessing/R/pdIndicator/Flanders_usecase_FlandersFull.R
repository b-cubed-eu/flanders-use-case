...
cube_path <- "data/Angiosperm_data_Flanders.csv"
...
#---------------------------------------------------------------------
# PRE-PROCESS CUBE - START
#---------------------------------------------------------------------

# Filter invasives out of occurrence cube

## Get invasive gbif keys
invasives_belgium <- read.csv("data/dwca-alien-plants-belgium-v1.10/taxon.txt",
                              sep="\t")
invasives_belgium_bb <-
  name_backbone_checklist(name = invasives_belgium$scientificName)
# write.csv(invasives_belgium_bb,"data/intermediate/invasives_Belgium_bb.csv")

# read in from file:
# invasives_belgium_bb <- read.csv('data/intermediate/invasives_Belgium_bb.csv',
...
belgium_all_observed_taxon <-
  read.csv('data/dwca-natuurpunt-natagora-checklist-v1.3/taxon.txt',
           stringsAsFactors = FALSE, sep = "\t")
belgium_all_observed_distr <-
  read.csv('data/dwca-natuurpunt-natagora-checklist-v1.3/distribution.txt',
           stringsAsFactors = FALSE, sep = "\t")
belgium_all_observed_distr <-
...
cube_native2 <- cube_native[cube_native$specieskey %in%
...
#-----------------------------------------------------------------------------
# PRE-PROCESSING CUBE - END
#-----------------------------------------------------------------------------
#-----------------------------------------------------------------------------
# CALCULATION PD CUBE - START
#-----------------------------------------------------------------------------
...
length(unique(cube_native2$specieskey)) # 1166 native angiosperm species in
                                        # final cube
...
# mcube <- read.csv('data/intermediate/mcube.csv', stringsAsFactors = FALSE,
# sep = ",")
mcube_orig <- mcube
...
write.csv(mcube,"data/intermediate/mcube_final.csv")
write.csv(pd_cube[, !(names(pd_cube) %in% c("specieskeys", "speciesnames",
          "verbatim_names"))], "pd_cube_Angiosperms_Flanders.csv")

#-----------------------------------------------------------------------------
# CALCULATION PD CUBE - END
#-----------------------------------------------------------------------------
...
# read in subsequent datafiles
pdcube_Flanders <- read.csv("output/pd_cube_Angiosperms_Flanders.csv")
pdcube_n2000 <- read.csv("output/pd_cube_Angiosperms_Flanders_N2000.csv")
pdcube_non2000 <- read.csv("output/pd_cube_Angiosperms_Flanders_nonN2000.csv")
pdcube_threatened <- read.csv("output/pd_cube_Angiosperms_Flanders_threatened.csv")

# generate pd loss cube

pd_map_loss <- generate_map_and_indicator(pd_cube_redlist, grid, "Redlist for Flanders")



# read

pdindicator <- generate_map_and_indicator(pd_cube, grid,
                                          "Angiosperms for Flanders")
pdindicator
