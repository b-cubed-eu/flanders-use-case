---
title: "Download and prepare Insecta data"
...
PARTITION BY orderkey, GBIF_EEARGCODE(10000, decimallatitude, decimallongitude, COALESCE(coordinateuncertaintyinmeters, 10000)), \"year\")) ordercount,
IF(ISNULL(familykey), NULL, SUM(COUNT(*)) OVER (
PARTITION BY familykey, GBIF_EEARGCODE(10000, decimallatitude, decimallongitude, COALESCE(coordinateuncertaintyinmeters, 10000)), \"year\")) familycount,
IF(ISNULL(genuskey), NULL, SUM(COUNT(*)) OVER (
PARTITION BY genuskey, GBIF_EEARGCODE(10000, decimallatitude, decimallongitude, COALESCE(coordinateuncertaintyinmeters, 10000)), \"year\")) genuscount,
...
data_total_10km <- download_occ_cube(
  sql_query = query_ang_10km,
...
cube_native <- data_total_10km[data_total_10km$specieskey %in%
...
```{r}
FLANDERS_EEA_10km <- st_read('./_preprocessing/data/raw/spatial/EEA_10km_Flanders_union.geojson')
```

```{r}
Cube_Angio_Flanders <- cube_native%>%
  filter(eeacellcode %in% FLANDERS_EEA_10km$CELLCODE)
```

```{r}
write.csv(Cube_Angio_Flanders,
          '_preprocessing/data/raw/Insecta_data_Flanders_10km.csv')
