...
b <- ggplot(presence_per_year, aes(x = year, y = n_present)) +
  geom_point(color = "darkred", size = 3) +
  geom_smooth(method = "loess", color = "steelblue", fill = "lightblue", se = TRUE) +
  scale_y_continuous(limits = c(0, 5)) +   # 🔥 dit is wat je zoekt
  labs(
    title = "Absolute occupancy of well serveyed cells",
    x = "Year",
    y = "Percentage (%)"
  ) +
  theme_minimal(base_size = 14)
...
korenbloem <- data_filtered%>% filter(specieskey== 3087403)

korenbloem_data_cube <- process_cube(korenbloem)

#trend in absolute occupancy over de tijd van korenbloem
plot_ts(spec_occ_ts(korenbloem_data_cube),
        title = 'Absolute number of occurences in well surveyed cells')
...
korenbloem_data_cube <- process_cube(bijenorchis)
