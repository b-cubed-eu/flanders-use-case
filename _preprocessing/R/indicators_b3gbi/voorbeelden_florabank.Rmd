...
```{r}
Focus <- 'Florabank_data_Flanders_1km'
...
```{r}
#only maintain cells which are well surveyed
data_filtered <- data %>%
...
# Stel df_filtered is je dataset met enkel goed onderzochte hokken

species_key <- 3089561

# Per jaar berekenen
presence_per_year <- data_filtered %>%
  group_by(year, eeacellcode) %>%           # per hok per jaar
  summarise(present = any(specieskey == species_key), .groups = "drop") %>%
  group_by(year) %>%                        # per jaar samenvatten
  summarise(
    n_cells = n(),                          # totaal aantal goed onderzochte hokken
    n_present = sum(present),               # aantal hokken waar de soort aanwezig is
    perc_present = 100 * n_present / n_cells  # percentage
  )

ggplot(presence_per_year, aes(x = year, y = perc_present)) +
  geom_point(color = "darkred", size = 3) +            # de bollen
  geom_smooth(method = "loess", color = "steelblue", fill = "lightblue", se = TRUE) +  # smooth curve
  labs(
    title = "Percentage goed onderzochte hokken met soort 3089561 per jaar",
    x = "Jaar",
    y = "Percentage (%)"
  ) +
...
korenbloem <- data%>% filter(specieskey== 3089561)

korenbloem_data_cube <- process_cube(korenbloem)

#trend in aantal observaties over de tijd van korenbloem
plot_ts(spec_occ_ts(korenbloem_data_cube))
...
```{r}
#trend in occupancy
bezette_hokken_per_jaar <- korenbloem %>%
  distinct(year, eeacellcode) %>%
  group_by(year) %>%
  summarise(aantal_eeacellcode = n())

library(ggplot2)

library(ggplot2)

ggplot(bezette_hokken_per_jaar, aes(x = year, y = aantal_eeacellcode)) +
  geom_point(size = 2) +                  # bolletjes
  geom_smooth(method = "loess", se = TRUE) +  # smooth curve
  labs(
    title = "Occupancy of Centaurea cyanus",
    x = "Year",
    y = "Occupancy"
  ) +
  theme_minimal()
```
```{r}
korensla <- data%>% filter(specieskey== 3087403)

korensla_data_cube <- process_cube(korensla)

#trend in aantal observaties over de tijd van korenbloem
plot_ts(spec_occ_ts(korensla_data_cube))

#trend in occupancy
bezette_hokken_per_jaar <- korensla %>%
  distinct(year, eeacellcode) %>%
  group_by(year) %>%
  summarise(aantal_eeacellcode = n())
...
ggplot(bezette_hokken_per_jaar, aes(x = year, y = aantal_eeacellcode)) +
  ...
  geom_smooth(method = "loess", se = TRUE) +  # smooth curve
  labs(
    title = "Occupancy of Arnoseris minima",
    x = "Year",
    y = "Occupancy"
  ) +
  theme_minimal()
