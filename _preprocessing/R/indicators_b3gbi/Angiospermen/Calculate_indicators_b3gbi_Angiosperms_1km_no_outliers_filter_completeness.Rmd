...
```{r}
Focus <- 'Flanders_1km_no_outliers_filter_completeness'
...
#block of code used to create the cube with filter for completeness, only run once.
```{r, eval=FALSE}
...
Focus <- 'Flanders_1km_no_outliers.csv'
...
Completeness <- ComP_map$data
Completeness_high <- Completeness%>%
  filter(diversity_val>0.5)

Cube_initial <- read.csv(paste0("./_preprocessing/data/raw/Angiosperm_data_Flanders_1km_no_outliers.csv"))

Final_Cube <- Cube_initial %>%
  filter(eeacellcode%in%Completeness_high$cellCode)

write.csv(Final_Cube, './_preprocessing/data/raw/Angiosperm_data_Flanders_1km_no_outliers_filter_completeness.csv')
