...
pd_map <- read_sf("./src/pdIndicator/pd_Vl_1km.geojson")

pd_map <- st_transform(pd_map, crs=3035)


ggplot(data = pd_map) +
  geom_sf(aes(fill = pd), color = NA) +
  scale_fill_viridis_c(na.value = NA,
                       name = "Faith’s PD") +   # voor continue waarden
  theme_void()

ggsave(
  filename = "./src/pdIndicator/pd.png",
  plot = a,
  width = 8,
  height = 5,
  dpi = 300
...
pd_map <- read_sf("./src/pdIndicator/pd_loss_Vl_1km.geojson")

pd_map <- st_transform(pd_map, crs=3035)

b <- ggplot(data = pd_map) +
  geom_sf(aes(fill = pd_loss), color = NA) +
  scale_fill_viridis_c(na.value = NA,
                       name= "Expected loss of Faith’s PD") +   # voor continue waarden
  theme_void()

ggsave(
  filename = "./src/pdIndicator/pdlossmap.png",
  plot = b,
  width = 8,
  height = 5,
  dpi = 300
