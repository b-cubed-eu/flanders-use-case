...
title:  >
  ...
  </a>General indicators at 10 km resolution
format:
  html:
    code-fold: true
...
```{r, echo = FALSE}
PEM_map <- readRDS('./Flanders_10km/PEM_map.rds')$data
...
```{r, echo = FALSE}
PEM_ts <- readRDS('./Flanders_10km/PEM_ts.rds')
plot(PEM_ts)
```
...
```{r, echo = FALSE}
WEM_map <- readRDS('./Flanders_10km/WEM_map.rds')$data
...
```{r, echo = FALSE}
WEM_ts <- readRDS('./Flanders_10km/WEM_ts.rds')
plot(WEM_ts)
```
:::
...
```{r, echo = FALSE}
HSh_map <- readRDS('./Flanders_10km/HSh_map.rds')$data
...
```{r, echo = FALSE}
HSh_ts <- readRDS('./Flanders_10km/HSh_ts.rds')
plot_ts(HSh_ts)
```
